<?php
    ...
    $header_grid2 = "";
    $hide_foot_widgets='';
    $footer_logo_size = !empty($reobiz_option['footer-logo-height']) ? 'style="height: '.$reobiz_option['footer-logo-height'].'"' : '';
    ...
    if ( class_exists( 'WooCommerce' ) && is_shop() || class_exists( 'WooCommerce' ) && is_product_tag()  || class_exists( 'WooCommerce' ) && is_product_category()  ) {
       ...
       $header_width_meta = get_post_meta($reobiz_shop_id, 'header_width_custom2', true);
       $hide_foot_widgets = get_post_meta($reobiz_shop_id, 'hide_foot_widgets', true);
       $footer_logo       = get_post_meta($reobiz_shop_id, 'footer_logo_img', true);
    ...
    if ($header_width_meta != ''){
        ...
        $header_width = !empty($reobiz_option['header-grid2']) ? $reobiz_option['header-grid2'] : '' ;
            ...
            ?>
                <?php if ( is_active_sidebar( 'footer'.( $i+1 ) ) ): ?>
                    <div class="<?php echo esc_attr( 'col-lg-' . $count ); ?> footer-<?php echo esc_attr($i); ?>">
                        ...
                        if( $i == 0) :                    
                            if($footer_logo !='' && empty($reobiz_option['enable_global'])){ ?>
                                <div class="footer-logo-wrap">
                                    <a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="footer-top-logo">
                                        <img <?php echo wp_kses($footer_logo_size, 'reobiz');?> src="<?php echo esc_url($footer_logo); ?>" alt="<?php echo esc_attr( get_bloginfo( 'name' )); ?>">
                                    </a>
                                </div>  
                            
                            <?php } else { 
                                if(!empty($reobiz_option['footer_logo']['url'])) { ?>
                                    <div class="footer-logo-wrap">
                                        <a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="footer-top-logo">
                                            <img <?php echo wp_kses($footer_logo_size, 'reobiz');?> src="<?php  echo esc_url($reobiz_option['footer_logo']['url'])?>" alt="<?php echo esc_attr( get_bloginfo( 'name' )); ?>">
                                        </a>
                                    </div>
                                <?php }
                            } endif;?>
